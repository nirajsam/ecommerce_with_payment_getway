...
function ProductsScreen(props) {
  ...
  const onChange = e =>{
    ...
    for (let index = 0; index < e.target.files.length; index++) {
      fileAr.push(e.target.files[index])
    }
    for (let index = 0; index < e.target.files.length; index++) {
      fileNameAr.push(e.target.files[index].name)
    }
    // setFile(e.target.files[0]);
    // setFilename(e.target.files[0].name);
    setFile(fileAr)
    setFilename(fileNameAr)
  ...
  return <div className="content content-margined">
        ...
        <form onSubmit={submitHandler} >
          <ul className="form-container">
            ...
            <li>
              ...
              <input type="text" name="name" value={name} id="name" autoFocus="autofocus" onChange={(e) => setName(e.target.value)}>
              </input>
            </li>
            <li>
              <label htmlFor="price">
            ...
            <li>
                <div className="form-group">
                    <input type='file' className="form-control form-control" id="customFile" onChange={onChange} multiple/>
                    {/* <label className="form-control" htmlFor="customFile">
                        {fileName}
                    </label> */}
                </div>
                <input type="button" value="upload" className="btn btn-primary btn-block mt-4" onClick={onSub}/>
            ...
            <li>
              ...
              <input type="text" name="name" value={image}  id="image" onChange={(e) => setImage(uploadFile.filePath)}>
              </input>
            </li>
            <li>
              <label htmlFor="brand">
    ...
    <div className="product-list" style={{"overflow-x":"auto"}}>
    ...
    <button onClick={()=>{return setorder(true)}}>orders</button>
    {order?<OrdersScreen/>:null}
  </div>
}
